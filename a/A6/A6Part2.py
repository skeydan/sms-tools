...
def hz2cent(f): return 1200 * np.log2((f + eps)/55.0)
def segSizeSec(segSize, H, fs): return segSize * H /fs
...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    #At each time frame (for each f0 value) you should compute the standard deviation of the
    ...
    stable = []
    while i < f0.size:
        if np.std(f0[i - winStable + 1 : i + 1]) < stdThsld:
            stable.append(i)
        i = i + 1
    len(stable)
    ...
    i = 1
    while i < len(starts):
        if segSizeSec(starts[i] - starts[i - 1], H, fs)  >= minNoteDur:
            segments.append([starts[i - 1], starts[i] - 1])
        i = i + 1
...
#first = segmentStableNotesRegions(inputFile='sounds/cello-phrase.wav', stdThsld=10, minNoteDur=0.1,
#winStable = 3, window='hamming', M=1025, N=2048, H=256, f0et=5.0, t=-100, minf0=310, maxf0=650)


# 
# segmentStableNotesRegions(inputFile='sounds/cello-phrase.wav', stdThsld=20, minNoteDur=0.5, 
# winStable = 3, window='hamming', M=1025, N=2048, H=256, f0et=5.0, t=-100, minf0=310, maxf0=650)
# 
# segmentStableNotesRegions(inputFile='sounds/sax-phrase-short.wav', stdThsld=5, minNoteDur=0.6, 
# winStable = 3, window='hamming', M=1025, N=2048, H=256, f0et=5.0, t=-100, minf0=310, maxf0=650)
