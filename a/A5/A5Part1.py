...
def minFreqEstErr(inputFile, f):
    ...
    while True:
        ...
        # True: periodic
        # False: symmetric
        # https://en.wikipedia.org/wiki/File:Comparison_of_symmetric_and_periodic_triangular_window_functions.svg
        # In the case of the 9-length window, that is done by combining its first and last coefficients
        # by addition (called periodic summation, with period 8)
        #
        w = get_window(window, M, False)
        center = 0.5 * x_.shape[0]
        x = x_[int(center - np.floor(M/2)) : int(center + np.floor(M/2) + 1)]
        N = int(np.power(2, np.ceil(np.log2(M))))
        mX, pX = DFT.dftAnal(x, w, N)
        ploc = UF.peakDetection(mX, t)
        iploc, ipmag, ipphase = UF.peakInterp(mX, pX, ploc)
        fEst = fs/N * iploc[0]
        diff = np.abs(f - fEst)
        #print(k, M, N, fEst, diff)
        if diff < 0.05: return(fEst, M, N)
        k = k +1
#
#minFreqEstErr('sounds/sine-490.wav', 490.0)
#minFreqEstErr('sounds/sine-1000.wav', 1000.0)
#minFreqEstErr('sounds/sine-200.wav', 200.0)
