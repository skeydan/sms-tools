...
def testRealEven(x):
    ...
    """
    M = x.shape[0]               # 7
    hM1 = int(np.floor((M+1)/2))         # 4
    hM2 = int(np.floor(M/2))             # 3
    dftbuffer = np.zeros(M)      # 
    dftbuffer[:hM1] = x[hM2:]    # 3,4,5,6 second part
    dftbuffer[-hM2:] = x[:hM2]   # 0,1,2 first part of signal
    X = fft(dftbuffer)
    #If f is real and even, |F [k]| = |F [N − k]| and imag(F [k]) = 0
    is_sym = np.all(X.real[1:M].transpose() == X.real[1:M])
    is_real = False if np.all(np.imag(X) > 1e-6) else True
    return(bool(is_sym and is_real), dftbuffer, X)
