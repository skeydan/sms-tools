...
def suppressFreqDFTmodel(x, fs, N):
    ...
    mX, pX = dftAnal(x, w, N)
    nykvist = fs/2
    bins_per_freq = (N/2)/nykvist
    bin_freq_70 = int(np.ceil(bins_per_freq * 70))
    filtX = mX.copy()
    filtX[0:(bin_freq_70 + 1)] = -120
    unfiltered = dftSynth(mX, pX, M) * outputScaleFactor
    filtered = dftSynth(filtX, pX, M) * outputScaleFactor
    return(unfiltered, filtered)
    
fs = 44100
t = 0.2
x = np.cos(2 * np.pi * 40 * np.arange(0, t, 1/fs)) + np.cos(2 * np.pi * 100 * np.arange(0, t, 1/fs)) + np.cos(2 * np.pi * 200 * np.arange(0, t, 1/fs)) + np.cos(2 * np.pi * 1000 * np.arange(0, t, 1/fs))
M = x.shape[0]
N = int(pow(2, np.ceil(np.log2(M)) + 1))
(unfiltered, filtered) = suppressFreqDFTmodel(x, fs, N)
