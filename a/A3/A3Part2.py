...
def optimalZeropad(x, fs, f):
    ...
    """
    period_in_samples = fs/f
    M = x.shape[0]
    num_periods_in_x = M/period_in_samples
    min_periods = np.ceil(num_periods_in_x)
    dft_size = min_periods * period_in_samples
    X = fft(x, n = int(dft_size))
    return(20 * np.log10(abs(X[ : int((dft_size/2)) + 1])))
...
# fs = 1000
# # generate x
# x = np.cos(2 * np.pi * f * np.arange(0, 1, 1/fs))[0:M]
